...
var getSortedNodes = function (nodes, options) {
    ...
    var splitBySideEffectNodes = nodes.reduce(function (chunks, node) {
        var type = get_chunk_type_of_node_1.getChunkTypeOfNode(node);
    ...
    for (var _i = 0, splitBySideEffectNodes_1 = splitBySideEffectNodes; _i < splitBySideEffectNodes_1.length; _i++) {
        ...
        if (chunk.type === constants_1.chunkTypeUnsortable) {
            ...
            var sorted = get_sorted_nodes_by_import_order_1.getSortedNodesByImportOrder(chunk.nodes, options);
    ...
    adjust_comments_on_sorted_nodes_1.adjustCommentsOnSortedNodes(nodes, finalNodes);
    return finalNodes;
};
